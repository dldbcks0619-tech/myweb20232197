...
async function fetchFromAIAPI(query) {
    // 보안을 위해 Vercel Serverless Function(/api/chat) 호출
    // 더 이상 프론트엔드에 API 키를 노출하지 않습니다.
    const url = '/api/chat';
    ...
    try {
        ...
        // 로컬에서 파일 더블클릭으로 실행 시 서버리스 함수 작동 불가
        if (window.location.protocol === 'file:') {
            return '[테스트 환경 오류] 현재 로컬 파일 상태로 실행 중이므로 보안 백엔드(Vercel) 서버리스 함수에 연결할 수 없습니다. 배포된 주소에서 테스트해 주세요.';
        }
